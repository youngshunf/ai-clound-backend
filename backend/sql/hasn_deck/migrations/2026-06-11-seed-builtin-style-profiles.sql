-- =====================================================
-- 演示文稿 hasn_deck.style_profile builtin 预置（37 风格，云端权威 / owner_id='system' / source='builtin'）
...
INSERT INTO "hasn_deck"."style_profile" (slug, label, description, source, design_contract, style_prompt, owner_id, rev, created_time)
...
ON CONFLICT (owner_id, slug) WHERE deleted_time IS NULL DO UPDATE SET
  ...
  rev = "hasn_deck"."style_profile".rev + 1, updated_time = now();
