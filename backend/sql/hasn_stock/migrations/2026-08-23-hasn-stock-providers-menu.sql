...
--
-- 【为什么会有这个文件】与同批的 2026-08-23-hasn-stock-providers-schema-and-seed.sql 同因：
--   原文件 backend/sql/hasn_stock/hasn_stock_providers_menu.sql 不在 migrations/ 子目录下，
--   生产迁移 runner 只扫 */migrations/*.sql，因此它从未在生产执行过——线上 sys_menu 里
--   既没有「素材站目录」菜单、也没有 hasn_stock:provider:add/edit/del 三个按钮权限，
--   管理员在后台**看不到这个页面**，也就无从为 pexels/pixabay 填 api_key。
--   本文件把同一份菜单种子搬进 migrations/，使其真正纳入部署流程。
--
-- 内容与原文件逐字一致（幂等键：菜单按 name/perms 唯一定位），仅补上本段说明。
