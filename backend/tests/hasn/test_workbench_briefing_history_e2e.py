...
覆盖本轮新增（工作台历史 + dismiss 持久化）：
  - GET /workbench/briefing/latest 今日视图：dismiss 后刷新不再出现被忽略项（服务端 join 反馈过滤）；
  ...
  - GET /workbench/briefing/history：按 period 倒序列出，含 focus_count/plan_count/summary 预览。
  - owner 隔离：A 的历史不串到 B。
...
_BASE = '/api/v1/hasn/app/workbench/briefing'
