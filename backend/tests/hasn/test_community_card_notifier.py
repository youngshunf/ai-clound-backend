...
覆盖三层（不依赖 PG，纯逻辑 + 路由接缝；route_message 真实行为由 message_router 自有测试覆盖）：
1. 卡片体构造——schema 必须过 validate_card_message_body，URI/资源类型/primary_action 正确。
2. 路由接缝——notify_* 必须以 from=分身、to=主人、content_type=5（卡片）调 route_message，
   且 content 即构造出的卡片体；local_id 幂等键存在。
...
def test_build_post_card_is_valid_and_links_to_detail() -> None:
    card = notifier.build_community_resource_card(
        'post', 'p_abc123', title='今天的随想', summary='一段较长的正文预览……'
    )
    # 过 schema 自检（不抛即合法）。
    ...
    assert card['source']['kind'] == 'app'
    assert card['source']['id'] == 'community'
    assert card['source']['display_name'] == '社区'
...
def test_build_article_card_is_valid_and_links_to_detail() -> None:
    card = notifier.build_community_resource_card(
        'article', 'art_xyz789', title='深度长文', summary='文章摘要'
    )
    validate_card_message_body(card)
    assert card['resource']['type'] == 'community.article'
    assert card['resource']['uri'] == 'hasn://community/articles/art_xyz789'
    assert card['primary_action']['uri'] == 'hasn://community/articles/art_xyz789'
    assert card['primary_action']['action_id'] == 'open_community_article'
    assert card['primary_action']['event']['payload'] == {'article_id': 'art_xyz789'}


def test_post_card_title_falls_back_when_content_blank() -> None:
    # 正文为空白：标题不能空（schema 要求非空），回落「社区帖子」。
    card = notifier.build_community_resource_card('post', 'p_blank', title='社区帖子', summary='')
    validate_card_message_body(card)
    assert card['title'] == '社区帖子'
...
async def test_notify_post_routes_card_to_owner(captured_route: dict[str, Any]) -> None:
    await notifier.notify_owner_post_card(
        agent_hasn_id=_AGENT, owner_hasn_id=_OWNER, post_id='p_abc123', content='正文内容很长很长'
    )
...
async def test_notify_article_routes_card_to_owner(captured_route: dict[str, Any]) -> None:
    ...
    assert captured_route['content']['resource']['uri'] == 'hasn://community/articles/art_xyz789'
    assert captured_route['content']['title'] == '深度长文'


@pytest.mark.asyncio
async def test_notify_skips_when_identities_missing(captured_route: dict[str, Any]) -> None:
    await notifier.notify_owner_post_card(
        agent_hasn_id='', owner_hasn_id=_OWNER, post_id='p_x', content='x'
    )
...
async def test_notify_is_best_effort_on_route_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 不抛即通过。
    await notifier.notify_owner_post_card(
        agent_hasn_id=_AGENT, owner_hasn_id=_OWNER, post_id='p_x', content='x'
    )
