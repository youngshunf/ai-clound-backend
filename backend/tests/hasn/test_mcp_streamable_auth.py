...
async def test_streamable_rejects_revoked_agent_jwt_before_agent_lookup(
    ...
    streamable_module = sys.modules['backend.app.mcp.streamable']

    async def reject_revoked_token(_token: str) -> NoReturn:  # ruff: ignore[unused-async]
    ...
    get_agent = AsyncMock()
    monkeypatch.setattr(
        streamable_module.hasn_agents_dao,
        'get_by_hasn_id',
        get_agent,
    )
...
async def test_streamable_binds_daemon_work_session_header_to_agent_context(
    ...
    observed_work_session_ids: list[str | None] = []
    ...
    class CapturingSessionManager:
        async def handle_request(self, *_args: object) -> None:
            ...
            observed_work_session_ids.append(current_context.work_session_id)

    server = HasnMcpStreamableServer()
    server.session_manager = cast('Any', CapturingSessionManager())
    monkeypatch.setattr(server, '_authenticate_from_headers', AsyncMock(return_value=context))

    async def receive() -> dict[str, object]:  # ruff: ignore[unused-async]
        return {'type': 'http.request', 'body': b'', 'more_body': False}

    async def send(_message: object) -> None:  # ruff: ignore[unused-async]
        return None
    ...
    assert observed_work_session_ids == ['sess_cli_artifact_1']
