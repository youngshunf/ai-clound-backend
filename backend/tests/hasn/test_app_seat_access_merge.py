...
async def _seed_enterprise_entitlement(db, *, app_id: str, enterprise_id: int, seats_total: int | None) -> None:
...
async def test_enterprise_subject_reads_enterprise_entitlement_not_owner(db) -> None:
    ...
    db.add(HasnAppSeat(
        entitlement_id=0, enterprise_id=eid, app_id=app_id,
        member_hasn_id=owner, assigned_by='admin', status='assigned',
    ))
    await db.flush()
    access = await app_catalog_service.resolve_app_access(
        db, catalog=cat, owner_hasn_id=owner,
        subject_type='enterprise', subject_id=str(eid), member_hasn_id=owner,
    )
    assert access['allowed'] is True
    assert access['reason'] == 'entitled'


async def test_owner_dimension_unchanged_when_subject_id_omitted(db) -> None:
...
async def test_enterprise_bought_no_seat_needs_assignment(db) -> None:
    app_id = f'ent_{_uid()}'
    eid = 700_000 + int(_uid(), 16) % 100_000
    cat = _catalog(app_id)
    db.add(cat)
...
async def test_enterprise_not_bought_needs_purchase(db) -> None:
    app_id = f'ent_{_uid()}'
    eid = 700_000 + int(_uid(), 16) % 100_000
    cat = _catalog(app_id)
    db.add(cat)
...
async def test_s1_free_enterprise_app_no_seat_gate(db) -> None:
...
async def test_s1_enterprise_entitlement_without_seats_is_entitled(db) -> None:
...
def test_merge_enterprise_allowed_wins() -> None:
    merged = app_catalog_service.merge_access(_acc(False, 'need_purchase', 'purchase'), _acc(True, 'entitled'))
    assert merged['allowed'] is True
    assert merged['reason'] == 'entitled'


def test_merge_owner_allowed_when_enterprise_denies() -> None:
    merged = app_catalog_service.merge_access(_acc(True, 'entitled'), _acc(False, 'need_seat_assignment', 'seat'))
    assert merged['allowed'] is True
    assert merged['reason'] == 'entitled'
...
def test_merge_self_serve_priority_need_purchase_over_seat() -> None:
    ...
    merged = app_catalog_service.merge_access(
        _acc(False, 'need_purchase', 'purchase'), _acc(False, 'need_seat_assignment', 'seat')
    )
...
def test_merge_seat_when_owner_not_self_serve() -> None:
    ...
    merged = app_catalog_service.merge_access(
        _acc(False, 'disabled'), _acc(False, 'need_seat_assignment', 'seat')
    )
...
def test_merge_none_enterprise_returns_owner() -> None:
    ...
    owner = _acc(False, 'need_enterprise_space', 'enterprise_space')
