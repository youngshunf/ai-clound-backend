...
async def test_project_artifact_flow_includes_kb_documents(session) -> None:
    ...
    assert not [row for row in before if row.get('resource_uri') in {kb_uri, doc_uri}]

    await project_linkage_registry.link(session, owner=owner, resource_uri=kb_uri, project_id=project.id)
    after = {row.get('resource_uri') for row in
             await project_service.project_artifact_flow(session, owner=owner, project_id=project.id)}
    ...
    restored = {row.get('resource_uri') for row in
                await project_service.project_artifact_flow(session, owner=owner, project_id=project.id)}
