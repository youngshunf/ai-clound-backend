...
async def _seed_project(session: AsyncSession, *, owner_id: str) -> str:
    """创建场景实例化所需的云端权威平台项目。"""
    project = HasnProject(owner_id=owner_id, name='工作流模板测试项目', status='active')
    session.add(project)
    await session.flush()
...
def _valid_graph() -> dict:
    """迷你合法蓝图：起点 idea + agent 节点 research（apps/resource_kind 都取真实注册值）。

    doc35：起点节点**不声明 output_spec**——它是主人输入的内联锚点、预完成为 done、永不过产出闸，
    旧的 `{kind:'workflow_anchor'}` 是给一个不参与匹配的位置硬造 kind，已随 §3 六枚举一起退役。
    """
    return {
        'nodes': [
            {
            ...
            },
            {
                ...
                # 旧写法 `{kind:'dataset'}`：`dataset` 已砍，且它本就答不出「要的是知识库」。
                'output_spec': {'required': True, 'label': '调研', 'expects': [{'resource_kind': 'knowledge.base'}]},
...
def test_validate_graph_spec_rejects_legacy_output_shape() -> None:
    """旧形状 `{kind: ...}` 必须硬拒（doc35 B1）——静默接受 = 闸看着还在、实际已失效。"""
    graph = _valid_graph()
...
def test_validate_graph_spec_rejects_misspelled_resource_kind() -> None:
    """`knowledge.bass` 拼错 → **发布期**就拒。

    否则运行期闸拿它去比对，永远比不中：分身真建了库、产物也登记了，节点却卡着不动，
    而根因藏在模板一个字母里——这正是 doc35 要根除的那类故障。
    """
    graph = _valid_graph()
    graph['nodes'][1]['output_spec'] = {'required': True, 'expects': [{'resource_kind': 'knowledge.bass'}]}
    with pytest.raises(errors.RequestError, match='knowledge.bass'):
        validate_graph_spec(graph)


def test_validate_graph_spec_rejects_expect_with_both_kinds() -> None:
    graph = _valid_graph()
    graph['nodes'][1]['output_spec'] = {
        'required': True,
...
def test_validate_graph_spec_rejects_over_max_nodes() -> None:
    nodes = [{'node_key': 'origin', 'is_origin': True, 'apps': []}]
    nodes += [
        {
            'node_key': f'n{i}',
            'is_origin': False,
            'apps': [],
            'output_spec': {'required': True, 'expects': [{'artifact_kind': 'document'}]},
        }
...
async def test_instantiate_builds_cloud_workflow(env: SimpleNamespace) -> None:
    ...
    project_id = await _seed_project(env.session, owner_id=owner)
    ...
    result = await workflow_template_service.instantiate_template(
        ...
        agent=agent,
        template_key=key,
        params={'origin_input': '做一个 AI 记账 App', 'title': '我的一人公司', 'project_id': project_id},
    )
    try:
...
async def test_instantiate_paid_template_denied_without_entitlement(env: SimpleNamespace) -> None:
    ...
    with pytest.raises(McpToolError) as ei:
        await workflow_template_service.instantiate_template(
            env.session, agent=agent, template_key=key, params={'origin_input': '想法', 'project_id': project_id}
        )
    err = ei.value
...
async def test_instantiate_paid_template_allowed_with_entitlement(env: SimpleNamespace) -> None:
    ...
    result = await workflow_template_service.instantiate_template(
        env.session, agent=agent, template_key=key, params={'title': '已购一人公司', 'project_id': project_id}
    )
    try:
        assert result['template_key'] == key
...
async def test_instantiate_free_template_bypasses_paywall(env: SimpleNamespace) -> None:
    ...
    result = await workflow_template_service.instantiate_template(
        env.session, agent=agent, template_key=key, params={'project_id': project_id}
    )
    try:
        assert result['workflow_id']
        assert result['template_key'] == key
