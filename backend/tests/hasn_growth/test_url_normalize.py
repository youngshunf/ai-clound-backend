...
def _normalize(value: str) -> tuple[str, str, str | None]:
...
class TestNormalizeUrl:
    def test_lowercases_host_and_strips_www(self) -> None:
        normalized, url_hash, domain = _normalize('http://www.Example.COM/Path')
    ...
    def test_adds_scheme_when_missing(self) -> None:
        normalized, _, domain = _normalize('example.com/contact')
    ...
    def test_root_path_and_trailing_slash_unify(self) -> None:
        ...
        a = _normalize('https://example.com')[0]
        b = _normalize('https://example.com/')[0]
        assert a == b == 'https://example.com'

    def test_trailing_slash_stripped_on_subpath(self) -> None:
        assert _normalize('https://example.com/a/b/')[0] == 'https://example.com/a/b'

    def test_drops_fragment(self) -> None:
        assert _normalize('https://example.com/a#section')[0] == 'https://example.com/a'

    def test_drops_tracking_params_keeps_real(self) -> None:
        normalized = _normalize('https://example.com/p?utm_source=x&id=5&fbclid=abc&gclid=z')[0]
        assert normalized == 'https://example.com/p?id=5'  # 仅保留业务参数 id

    def test_sorts_query_params(self) -> None:
        # 参数顺序不同的同一资源规范化后一致
        a = _normalize('https://example.com/p?b=2&a=1')[0]
        b = _normalize('https://example.com/p?a=1&b=2')[0]
        assert a == b

    def test_same_resource_same_hash(self) -> None:
        h1 = _normalize('http://WWW.example.com/Page/?utm_source=ad')[1]
        h2 = _normalize('http://example.com/Page')[1]
        assert h1 == h2

    def test_different_resource_different_hash(self) -> None:
        assert _normalize('https://example.com/a')[1] != _normalize('https://example.com/b')[1]

    def test_strips_port_in_domain(self) -> None:
        _, _, domain = _normalize('https://example.com:8080/a')
        assert domain == 'example.com'
