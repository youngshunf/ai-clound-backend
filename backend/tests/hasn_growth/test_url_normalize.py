...
def normalize_required(raw: str) -> tuple[str, str, str | None]:
    """断言有效输入的规范化结果存在，并向类型检查器收窄返回值。"""
...
class TestNormalizeUrl:
    def test_lowercases_host_and_strips_www(self) -> None:
        normalized, url_hash, domain = normalize_required('http://www.Example.COM/Path')
    ...
    def test_adds_scheme_when_missing(self) -> None:
        normalized, _, domain = normalize_required('example.com/contact')
    ...
    def test_root_path_and_trailing_slash_unify(self) -> None:
        ...
        a = normalize_required('https://example.com')[0]
        b = normalize_required('https://example.com/')[0]
        assert a == b == 'https://example.com'

    def test_trailing_slash_stripped_on_subpath(self) -> None:
        assert normalize_required('https://example.com/a/b/')[0] == 'https://example.com/a/b'

    def test_drops_fragment(self) -> None:
        assert normalize_required('https://example.com/a#section')[0] == 'https://example.com/a'

    def test_drops_tracking_params_keeps_real(self) -> None:
        normalized = normalize_required('https://example.com/p?utm_source=x&id=5&fbclid=abc&gclid=z')[0]
        assert normalized == 'https://example.com/p?id=5'  # 仅保留业务参数 id

    def test_sorts_query_params(self) -> None:
        # 参数顺序不同的同一资源规范化后一致
        a = normalize_required('https://example.com/p?b=2&a=1')[0]
        b = normalize_required('https://example.com/p?a=1&b=2')[0]
        assert a == b

    def test_same_resource_same_hash(self) -> None:
        h1 = normalize_required('http://WWW.example.com/Page/?utm_source=ad')[1]
        h2 = normalize_required('http://example.com/Page')[1]
        assert h1 == h2

    def test_different_resource_different_hash(self) -> None:
        assert normalize_required('https://example.com/a')[1] != normalize_required('https://example.com/b')[1]

    def test_strips_port_in_domain(self) -> None:
        _, _, domain = normalize_required('https://example.com:8080/a')
        assert domain == 'example.com'
