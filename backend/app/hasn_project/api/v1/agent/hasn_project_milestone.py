"""平台项目里程碑（v2·业务状态标记·无依赖无门控，doc38 §12.3） - Agent API
...
@router.get(
    '',
    summary='平台项目里程碑（v2·业务状态标记·无依赖无门控，doc38 §12.3）列表',
    dependencies=[DependsAgentJwtAuth],
    name='hasn_project_agent_list_hasn_project_milestone',
)
async def agent_list_hasn_project_milestone(
    request: Request,
    db: CurrentSession,
) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
    # 可以使用 agent.agent_hasn_id, agent.owner_hasn_id, agent.scopes
    data = await hasn_project_milestone_service.get_list(db=db)
...
@router.post(
    ...
    dependencies=[DependsAgentJwtAuth],
    name='hasn_project_agent_create_hasn_project_milestone',
)
async def agent_create_hasn_project_milestone(
    request: Request,
...
@router.get(
    ...
    dependencies=[DependsAgentJwtAuth],
    name='hasn_project_agent_get_hasn_project_milestone',
)
async def agent_get_hasn_project_milestone(
    request: Request,
    db: CurrentSession,
    ...
    # TODO: 根据实际业务需求添加权限检查
    # if hasn_project_milestone.owner_id != agent.owner_hasn_id:
...
@router.put(
    ...
    dependencies=[DependsAgentJwtAuth],
    name='hasn_project_agent_update_hasn_project_milestone',
)
async def agent_update_hasn_project_milestone(
    request: Request,
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='平台项目里程碑（v2·业务状态标记·无依赖无门控，doc38 §12.3） ID')],
...
@router.delete(
    ...
    dependencies=[DependsAgentJwtAuth],
    name='hasn_project_agent_delete_hasn_project_milestone',
)
async def agent_delete_hasn_project_milestone(
    request: Request,
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='平台项目里程碑（v2·业务状态标记·无依赖无门控，doc38 §12.3） ID')],
) -> ResponseModel:
