...
class HasnProjectSchemaBase(SchemaBase):
    ...
    cover_asset_uri: str | None = Field(None, description='封面图资产引用（hasn://asset/{id}，来源=上传/素材下载/AI 生成；序列化边界换 CDN 签名 URL，不存直链；可空回落品牌渐变+首字）')
    status: str = Field(description='状态 (active:进行中:blue/archived:已归档:gray)')
    bound_agent_id: str | None = Field(None, description='默认协作分身 HASN ID（owner 名下 a_* 分身，null=未绑定；对齐 doc21 AppCollab，列名铁律 doc38 §8）')
    enterprise_id: str | UUID | None = Field(None, description='企业归属（双模化，个人 NULL / 企业非空，对齐 GE，可空）')
