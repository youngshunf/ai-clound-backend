...
class ImCommandOutboxSchemaBase(SchemaBase):
    """社区资源写入触发主人知情卡的事务命令队列基础模型"""
    command_id: str = Field(description='命令公开标识')
    producer: str = Field(description='生产方固定标识 community')
    ...
    causation_id: str | None = Field(None, description='触发本命令的社区资源标识')
    completed_at: datetime | None = Field(None, description='投递完成时间')


class CreateImCommandOutboxParam(ImCommandOutboxSchemaBase):
    """创建社区资源写入触发主人知情卡的事务命令队列参数"""


class UpdateImCommandOutboxParam(ImCommandOutboxSchemaBase):
    """更新社区资源写入触发主人知情卡的事务命令队列参数"""


class DeleteImCommandOutboxParam(SchemaBase):
    """删除社区资源写入触发主人知情卡的事务命令队列参数"""

    pks: list[int] = Field(description='社区资源写入触发主人知情卡的事务命令队列 ID 列表')


class GetImCommandOutboxDetail(ImCommandOutboxSchemaBase):
    """社区资源写入触发主人知情卡的事务命令队列详情"""
