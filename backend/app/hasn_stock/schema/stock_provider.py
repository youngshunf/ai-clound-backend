...
class CreateProviderParam(SchemaBase):
    ...
    api_key: str | None = Field(default=None, description='明文 api_key（加密落库；不回显）')
    download_domains: list[str] = Field(default_factory=list, description='下载直链合法域名')
    enabled: bool = True
    priority: int = 100
    license_terms_url: str | None = Field(default=None, max_length=500)
...
class UpdateProviderParam(SchemaBase):
    ...
    api_key: str | None = Field(default=None, description='明文 api_key（传空串清空 key，None 不改）')
