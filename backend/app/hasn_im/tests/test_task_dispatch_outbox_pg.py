...
async def task_env():
    ...
    try:
        ...
        async with session_factory.begin() as db:
            await db.execute(
                sa.text(
                    'DELETE FROM hasn_task.task_dispatch_outbox '
            ...
            await db.execute(
                sa.text(
                    'DELETE FROM hasn_task.skill_bundle WHERE owner_id = :owner_id'
                ),
                {'owner_id': owner_id},
            )
...
async def _load_task(db, task_id: int) -> HasnTask:
    """从新会话读取任务，避免回滚后的 ORM 状态污染后续断言。"""
    return (
        await db.execute(select(HasnTask).where(HasnTask.id == task_id))
    ).scalar_one()


async def test_task_run_and_dispatch_command_share_one_transaction(task_env) -> None:
        ...
        run_count = int(
            (
                await db.execute(
                    ...
                    .select_from(HasnTaskRun)
                    .where(HasnTaskRun.task_id == task.id)
                )
            ).scalar_one()
        )
        outbox_count = int(
            (
                await db.execute(
                    select(sa.func.count())
                    .select_from(TaskDispatchOutbox)
                    .where(TaskDispatchOutbox.task_id == task.id)
                )
            ).scalar_one()
        )
...
async def test_committed_task_dispatch_recovers_via_real_redis(task_env) -> None:
    ...
    async with session_factory() as db:
        run = (
            await db.execute(
                select(HasnTaskRun).where(HasnTaskRun.task_id == task_env['task_id'])
            )
        ).scalar_one()
        outbox = (
            await db.execute(
                select(TaskDispatchOutbox).where(
                    TaskDispatchOutbox.task_id == task_env['task_id']
                )
            )
        ).scalar_one()
    ...
    async with session_factory() as db:
        completed = (
            await db.execute(
                select(TaskDispatchOutbox).where(
                    TaskDispatchOutbox.command_id == outbox.command_id
                )
            )
        ).scalar_one()
...
async def test_task_outbox_model_uses_private_schema_and_has_no_generic_routes() -> None:
    ...
    assert all(
        'dispatch/outbox' not in route.path
