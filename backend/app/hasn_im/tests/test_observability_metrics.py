...
3. **可见性**：13 个指标都注册进 prometheus 默认 REGISTRY（经 /metrics 可导出，即「演练环境可见」）；
...
_FORBIDDEN_ID_LABELS = {
    'owner_id', 'conversation_id', 'hasn_id', 'agent_id', 'message_id',
    'event_id', 'user_id', 'aggregate_id', 'source_event_id', 'trace_id',
    'owner', 'conversation', 'hasn', 'message', 'aggregate',
}


def _exposed_name(metric) -> str:
...
def test_metric_set_matches_spec_exactly() -> None:
    ...
    assert actual == set(EXPECTED), (
        f'§12.2 指标集漂移：缺 {set(EXPECTED) - actual}，多 {actual - set(EXPECTED)}'
    )
...
def test_all_registered_in_default_registry() -> None:
    """13 个指标都在默认 REGISTRY（经 /metrics 可导出 = 演练环境可见）。"""
...
def test_initial_thresholds_present() -> None:
    ...
    # 关键数值门槛与 §12.2 一致
    assert m.INITIAL_THRESHOLDS['sync_projection_lag_p99_seconds'] == 5.0
    assert m.INITIAL_THRESHOLDS['integration_consumer_oldest_pending_seconds'] == 30.0
