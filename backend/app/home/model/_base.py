"""home 应用独立 PG schema 基类。

每个 AI-Native 应用使用独立 PG schema（ADR-15：AI-Native 应用命名空间与目录约定）。
PG schema 统一用 `hasn_` 前缀。本模块由 `app/hasn` 中的工作台首页
简报/偏好子域（hasn_workbench_briefing / hasn_workbench_briefing_feedback / hasn_owner_workbench_pref）
按 ADR-15 §4 抽出为独立应用 app/home；端点路径已统一到 `/api/v1/hasn/app/home/*`（首页偏好/简报）
与 `/api/v1/hasn/app/apps/*`（应用目录/启动器/权益）。
本基类在 fba `Base`（bigint `id_key` + DateTimeMixin）之上注入 `schema='hasn_workbench'`，
使各 model 继承它即落到 `hasn_workbench.*` schema（持久化 schema/表名沿用旧名，未随代码重命名迁移）。

平台底座留 app/hasn（不随本应用迁出）：workbench_domain_service（应用域/工作空间解析，
被 enterprise/knowledge/workspace/ai_native 共用）、app_catalog_registry（ADR-15 M5 待退役）、
app_event_bus；内置任务目录服务已在 app/hasn_task（本应用 API 直连）。身份 public.hasn_agents
跨 schema 全限定引用。
