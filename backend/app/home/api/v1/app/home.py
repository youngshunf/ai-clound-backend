...
from backend.app.home.schema.workbench_briefing_document import (
...
@router.get('/home/pref', dependencies=[DependsJwtAuth], summary='读工作台偏好（主脑 + 简报设置）')
...
@router.put('/home/pref', dependencies=[DependsJwtAuth], summary='改工作台偏好（主脑 / 简报开关·时刻·数据源）')
...
@router.get('/home/builtin-tasks', dependencies=[DependsJwtAuth], summary='读官方内置任务目录（daemon 拉取播种）')
...
@router.get('/home/briefing/history', dependencies=[DependsJwtAuth], summary='历史简报列表（按日倒序，归档往期）')
...
@router.post(
    '/home/briefing/items/{item_id}/dismiss',
...
# 工作台展示统一走下面的 GET /apps（catalog ∩ entitlement）。


@router.get('/apps', dependencies=[DependsJwtAuth], summary='工作台全部已注册应用（注册即用）')
async def list_apps(request: Request, db: CurrentSession) -> ResponseModel:
    # 应用平台 v3（去工作空间绑定）：工作台清单与激活空间无关，故不再接收 workspace_kind。
    apps = await workbench_domain_service.list_apps(db, user_id=request.user.id)
...
# 应用开箱即用，无挂载开关；付费墙在 GET /apps 的 access 字段 + invoke 时把关。
