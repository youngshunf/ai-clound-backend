...
class ExternalMcpServer(ExternalMcpAppBase):
    ...
    args: Mapped[list[Any]] = mapped_column(postgresql.JSONB(), default_factory=list, comment='本机启动参数 jsonb（local_process）')
    ...
    advertised_tools_cache: Mapped[list[dict[str, Any]]] = mapped_column(postgresql.JSONB(), default_factory=list, comment='自省到的第三方工具缓存 jsonb（tools/list 归一 ToolMeta[]，目录层缓存非调用缓存）')
