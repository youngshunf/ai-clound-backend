...
class Playbook(HasnGrowthAppBase):
    ...
    version: Mapped[int] = mapped_column(sa.INTEGER(), default=1, comment='当前打法版本')
    ...
    cadence: Mapped[dict] = mapped_column(
        postgresql.JSONB(), default_factory=dict, comment='触达节奏 [{day,channel,goal}]'
    )
    tone_guide: Mapped[str | None] = mapped_column(UniversalText, default=None, comment=None)
    exit_rule: Mapped[dict] = mapped_column(
        postgresql.JSONB(), default_factory=dict, comment='止损规则 {max_silent_rounds,action}'
    )
    ...
    enterprise_id: Mapped[int | None] = mapped_column(
        sa.BIGINT(), default=None, comment='企业 ID（enterprise 模式；personal/内置 为 NULL）'
    )
