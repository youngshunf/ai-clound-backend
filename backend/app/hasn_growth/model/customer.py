...
class Customer(HasnGrowthAppBase):
    ...
    source_kind: Mapped[str] = mapped_column(
        sa.String(24),
        default='',
        comment='来源 (outbound_crawl:采集:blue/inbound_form:留资:green/community:内容:purple/manual:手动:gray)',
    )
    ...
    profile_json: Mapped[dict] = mapped_column(
        postgresql.JSONB(), default_factory=dict, comment='AI 画像（行业/规模/痛点/预算信号/决策角色/沟通偏好）'
    )
    ...
    lifecycle_status: Mapped[str] = mapped_column(
        ...
        default='',
        comment=(
            '生命周期 (active:跟进中:blue/engaged:有回应:cyan/opportunity:已立商机:purple/'
            'silent:沉默:gray/won:成交:green/lost:流失:red/archived:归档:gray)'
        ),
    )
    ...
    followup_task_id: Mapped[str | None] = mapped_column(
        sa.String(64), default=None, comment='当前跟进任务（hasn_task.task.id 逻辑引用）'
    )
    ...
    assignee: Mapped[str | None] = mapped_column(
        sa.String(64), default=None, comment='负责人 hasn_id（enterprise 模式跟进人；与 owner_agent_id 分身维度并存）'
    )
