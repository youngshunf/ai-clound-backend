...
class OutreachMessageSchemaBase(SchemaBase):
    ...
    approval_status: str | None = Field(None, description='审批状态；存量迁移完成前可空')
    delivery_status: str | None = Field(None, description='投递状态；存量迁移完成前可空')
    approval_version: int | None = Field(None, description='当前批准对应的内容版本')
    content_version: int | None = Field(None, description='当前内容版本')
    ...
    manual_attested_at: datetime | None = Field(None, description='人工完成证明时间')
    manual_attested_by: str | None = Field(None, description='人工证明操作人')
    manual_attested_channel: str | None = Field(None, description='人工证明使用的渠道')
