...
class OutreachMessageSchemaBase(SchemaBase):
    ...
    channel: str = Field(description='渠道 (manual_assist:人工辅助:gray/wechat:微信:green/qq:QQ:blue/feishu:飞书:cyan/email:邮件:orange/hasn_dm:站内:purple)')
    subject: str | None = Field(None, description='None')
    content: str = Field(description='None')
    content_assets: dict = Field(description='None')
    status: str = Field(description='状态 (draft:草稿:gray/pending_approval:待审批:orange/approved:已批准:blue/sending:发送中:cyan/sent:已发送:green/replied:已回复:purple/rejected:已拒绝:red/failed:失败:red/blocked_optout:退订拦截:red/blocked_compliance:合规拦截:red)')
    ...
    auto_approved: bool = Field(description='白名单放行标记（审计区分人批/自动）')
