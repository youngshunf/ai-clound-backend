...
class ProfileSchemaBase(SchemaBase):
    ...
    keywords: list[str] = Field(description='None')
    content_pillars: list[str] = Field(description='内容支柱 ["食谱教程","厨房好物","探店"]')
    posting_frequency: str | None = Field(None, description='None')
    best_posting_time: str | None = Field(None, description='None')
    style_references: list[str] = Field(description='None')
    taboo_topics: list[str] = Field(description='禁区话题（合规红线硬过滤，§12）')
