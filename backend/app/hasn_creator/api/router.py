...
#
# 【codegen 运营面已下线】AI-Native 应用移出平台归属：每个应用经 SDK 接入、自选语言、**自建业务
# 运营面**，云端后台不再按表生成 admin CRUD。原 14 张表的 codegen admin API 已整体删除，此处仅保留
# 空的 v1 以维持 backend/app/router.py 的装载契约（`from ... import v1`）不变。
# service/crud/model 全部保留——agent / open / app 三面仍在复用。
# ========================================
