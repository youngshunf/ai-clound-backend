...
class ReleaseAsset(HasnReleaseAppBase):
    ...
    platform_target: Mapped[str] = mapped_column(sa.String(32), default='', comment='平台目标（darwin-aarch64/darwin-x86_64/windows-x86_64/linux-x86_64/headless-linux-amd64/headless-linux-arm64）')
    asset_kind: Mapped[str] = mapped_column(sa.String(16), default='', comment='包类型 (installer:安装包dmg:blue/updater:热更新包:purple/image:容器镜像:cyan)')
    download_url: Mapped[str] = mapped_column(UniversalText, default='', comment='七牛 CDN 下载地址（https 直链）；asset_kind=image 时存私有 registry ref')
    ...
    # 列宽 128：docker digest 形如 sha256:<64hex> 共 71 字符，桌面端仍存纯 hex
    sha256: Mapped[str | None] = mapped_column(sa.String(128), default=None, comment='文件 sha256；asset_kind=image 时存镜像 digest')
