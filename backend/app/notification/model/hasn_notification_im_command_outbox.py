...
class HasnNotificationImCommandOutbox(Base):
    """通知业务状态触发 IM 卡片的事务命令队列"""

    __tablename__ = 'hasn_notification_im_command_outbox'

    id: Mapped[id_key] = mapped_column(init=False)
    command_id: Mapped[str] = mapped_column(sa.String(40), default='', comment='命令公开标识')
    producer: Mapped[str] = mapped_column(sa.String(40), default='', comment='生产方固定标识 notification')
    conversation_id: Mapped[str | UUID] = mapped_column(sa.UUID(), default=None, comment='ensure 后取得的权威会话 ID')
    ...
    payload_hash: Mapped[str] = mapped_column(sa.CHAR(), default='', comment='规范化命令载荷 SHA-256，用于同键异载荷冲突检测')
    idempotency_key: Mapped[str] = mapped_column(sa.String(160), default='', comment='跨 relay 重试稳定的 IM 幂等键')
    ...
    causation_id: Mapped[str | None] = mapped_column(sa.String(80), default=None, comment='触发本命令的业务事实标识')
