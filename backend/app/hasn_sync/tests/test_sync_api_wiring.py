...
_RECEIPT_MIGRATION = _BACKEND_ROOT / 'sql/hasn/migrations/2026-07-27-r3-sync-business-receipts.sql'
...
def _annotation(path: Path, function_name: str, argument_name: str) -> str:
    ...
    argument = next(item for item in function.args.args if item.arg == argument_name)
...
def test_general_sync_routes_use_only_sync_sessions() -> None:
    ...
    assert _annotation(_SYNC_API, 'push_sync_events', 'db') == 'CurrentSyncSessionTransaction'
    assert _annotation(_SYNC_API, 'pull_memory_sync_events', 'db') == 'CurrentSyncSession'
...
def test_conversation_projection_reads_use_im_session() -> None:
    ...
    assert _annotation(_SYNC_API, 'batch_get_conversation_objects', 'db') == 'CurrentImSession'


def test_message_history_bootstrap_uses_explicit_sync_and_im_sessions() -> None:
    """快照边界读 sync 流头，历史分页只读 IM 事实，禁止借 Python 通用角色。"""
    assert _annotation(_SYNC_API, 'start_message_history_bootstrap', 'sync_db') == 'CurrentSyncSession'
    assert _annotation(_SYNC_API, 'start_message_history_bootstrap', 'im_db') == 'CurrentImSessionTransaction'
    assert _annotation(_SYNC_API, 'page_message_history_conversations', 'db') == 'CurrentImSession'
    assert _annotation(_SYNC_API, 'page_message_history_messages', 'db') == 'CurrentImSession'


def test_task_sync_routes_use_only_sync_sessions() -> None:
    """任务同步入口只能连接 sync 自有域，禁止借 Python 角色跨域读表。"""
    assert _annotation(_TASK_SYNC_API, 'push_task_sync_events', 'db') == 'CurrentSyncSessionTransaction'
    assert _annotation(_TASK_SYNC_API, 'pull_task_sync_events', 'db') == 'CurrentSyncSession'
...
def test_task_pull_does_not_read_business_assignment_table() -> None:
    ...
    gateway_class = next(
        node for node in tree.body if isinstance(node, ast.ClassDef) and node.name == 'SqlAlchemySyncGateway'
    )
    gateway = next(
        node
        for node in gateway_class.body
        if isinstance(node, ast.AsyncFunctionDef) and node.name == 'pull_task_events'
    )
...
def test_task_write_freezes_node_visibility_in_sync_event() -> None:
    ...
    writer = next(
        node
        for node in gateway_class.body
        if isinstance(node, ast.AsyncFunctionDef) and node.name == '_upsert_task_and_append_event'
    )
...
def test_internal_receipt_has_no_generated_http_crud_surface() -> None:
    ...
    offenders = [
        path for path in api_root.rglob('*.py') if 'hasn_sync_business_receipts' in path.read_text(encoding='utf-8')
    ]
    assert offenders == []
