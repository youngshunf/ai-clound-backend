"""个人技能库 agent API（Agent JWT / Agent MCP Key，SKILLSYNC-C2 难点5）。

私有个人技能的**带鉴权、直回字节**下载端点，供 hermes provisioning 跨设备物化。
区别于 open 端点 302 跳私有桶 URL（urllib 取私有技能会 401）：此处用 Agent 凭证校验
...
async def download_personal_skill(
    ...
    # owner 归属由已验证 Agent 凭证的 owner_user_id 决定：只能下载本 owner 名下的个人技能。
