"""用户云存储导出逐资产不可变快照 应用的 G6 统一资源权限门·资源适配器骨架（doc33 S3-5 codegen 生成）。
...
5. manifest（`hasn/manifest.py` 或平台工具声明）里，收本资源 id 的工具条目补 `resource_access`：
   `resource_access = [{'param': '<id入参名>', 'type': 'hasn_storage_export_items', 'need': 'viewer|editor|manager'}]`。
...
class HasnStorageExportItemsResourceAdapter:
    """用户云存储导出逐资产不可变快照 资源适配器：resource_type='hasn_storage_export_items'。"""

    resource_type = 'hasn_storage_export_items'
    # TODO(G6)：列出工具入参里指代本资源 id 的参数名（如 ('hasn_storage_export_items_id',)）；留空 = 不进 S3-2 别名集。
...
def register() -> None:
    ...
    adapter = HasnStorageExportItemsResourceAdapter()
