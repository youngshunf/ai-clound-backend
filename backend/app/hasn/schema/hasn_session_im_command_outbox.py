...
class HasnSessionImCommandOutboxSchemaBase(SchemaBase):
    """工作会话结果与应用完成卡的事务 IM 命令队列基础模型"""
    command_id: str = Field(description='命令公开标识')
    producer: str = Field(description='生产方固定标识 session')
    ...
    causation_id: str | None = Field(None, description='触发本命令的会话或资源标识')
    completed_at: datetime | None = Field(None, description='投递完成时间')


class CreateHasnSessionImCommandOutboxParam(HasnSessionImCommandOutboxSchemaBase):
    """创建工作会话结果与应用完成卡的事务 IM 命令队列参数"""


class UpdateHasnSessionImCommandOutboxParam(HasnSessionImCommandOutboxSchemaBase):
    """更新工作会话结果与应用完成卡的事务 IM 命令队列参数"""


class DeleteHasnSessionImCommandOutboxParam(SchemaBase):
    """删除工作会话结果与应用完成卡的事务 IM 命令队列参数"""

    pks: list[int] = Field(description='工作会话结果与应用完成卡的事务 IM 命令队列 ID 列表')


class GetHasnSessionImCommandOutboxDetail(HasnSessionImCommandOutboxSchemaBase):
    """工作会话结果与应用完成卡的事务 IM 命令队列详情"""
