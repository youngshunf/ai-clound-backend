...
async def _kb_related_uri_pairs(
    db: AsyncSession,
    owner: str,
    rows_by_project: dict[UUID, tuple[Any, ...]],
) -> list[tuple[UUID, str]]:
    """批量映射知识库文档 URI，供项目列表聚合复用。"""
    kb_to_project = {
        row.id: project_id
        for project_id, rows in rows_by_project.items()
        for row in rows
    }
    if not kb_to_project:
        return []
    rows = (
        await db.execute(
            sa.select(Document.id, Document.kb_id).where(
                Document.owner_id == owner,
                Document.kb_id.in_(kb_to_project),
                Document.deleted_time.is_(None),
            )
        )
    ).all()
    return [
        (kb_to_project[kb_id], _build_uri('knowledge.document', document_id))
        for document_id, kb_id in rows
    ]
...
project_linkage_registry.register(
    LinkageAdapter(
        ...
        related_resource_uri_pairs=_kb_related_uri_pairs,
    )
)
